...
export default function ChatUI({chatWidth, userInfo}) {
  // let el = document.getElementById('messages')
  const messageFix = () =>{
    // message is not working FIXIUT
  ...
  return (
    // `calc(100% - 230px)`
  <div id='Chat' style={{width: `${chatWidth}`}} className="flex-1 absolute right-0 p:2 sm:p-6 justify-between flex flex-col h-screen">
    <div className="flex-row-reverse flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
      <div className="relative flex flex-row-reverse items-center space-x-4">
          ...
          <div className="flex flex-col leading-tight">
            <div className="text-2xl mt-1 flex items-center">
                <span className="text-gray-700 mr-3">{userInfo.userName.length >= 20 ? userInfo.userName.slice(0, 18) + '...' : userInfo.userName}</span>
            </div>
            <span className="text-lg text-gray-600">{userInfo.isOnline ? 'Online' : 'Offiline'}</span>
          </div>
      </div>
    </div>
    ...
    <div className="border-t-2 border-gray-200 px-4 pt-4 mb-2 sm:mb-0">
      <div className="relative flex">
          ...
          <input type="text" placeholder="Write your message!" className="w-full focus:outline-none focus:placeholder-gray-400 text-gray-600 placeholder-gray-600 pl-5 bg-gray-200 rounded-lg py-3"/>
          <div className="bg-gray-200 rounded-r-lg absolute right-0 items-center inset-y-0 hidden sm:flex">
            ...
            <button type="button" className="rounded-r-lg inline-flex items-center justify-center px-4 py-3 transition duration-500 ease-in-out text-white bg-blue-500 hover:bg-blue-400 focus:outline-none">
